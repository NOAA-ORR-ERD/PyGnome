...
"Temperature" : {"Kelvin"  : ((1.0, 0.0),["K","degrees k","degree k","degrees kelvin","degree kelvin","deg k"]),
                 ...
                 "Fahrenheit"  : ((0.55555555555555558, (273.16*(9. / 5.) - 32.0) ),["F","degrees f","degree f","degrees farenheight","deg f"]),
                 },
...
def all_unit_names():
    """
    returns a dict of all primary unit names, mapped to their type
    """
    for key, value in ConvertDataUnits.items():
        f.write( "\n%s:\n"%key )
        for key in value:
