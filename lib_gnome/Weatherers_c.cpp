               ...
               double drop_max)
{
    ...
    for (int i=0; i < n; i++)
    {
        ...
        if ((le_age >= emul_time && emul_time >= 0.) || (frac_evap[i] >= emul_C && emul_C > 0.))
        {
            ...
            //S = S + k_emul[i] * step_len * exp( (-k_emul[i] / S_max) * (age[i] - start));
            S = S + k_emul[i] * step_len * exp( (-k_emul[i] / S_max) * (le_age - start));
            if (S > S_max)
                S = S_max;
        }
        else
                      ...
                      double ka)
{
    ...
    for (int i=0; i < n; i++)
    {
        ...
        double De = disp_wave_energy[i];
        double fbw = frac_breaking_waves[i];
        double Hrms = wave_height[i];
        double C_disp = pow(disp_wave_energy[i], 0.57)* fbw;
