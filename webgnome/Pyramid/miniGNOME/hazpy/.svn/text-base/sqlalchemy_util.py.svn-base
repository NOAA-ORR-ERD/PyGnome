...
class SALogger(object):
    ...
    Usage::
        sql_log = SALogger()
        sql_log.initialize(does_user_want_sql_logging)
        sql_log.disable()
    ...
    and the ``@disable`` decorator. These are used to temporary disable logging
...
class MultiString(sa.types.TypeDecorator):
    """Same as MultiText but for VARCHAR columns."""
