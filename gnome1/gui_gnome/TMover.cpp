...
OSErr TMover::InitMover()
{
	return noErr;
}

OSErr TMover::UpItem(ListItem item)
{
	long i;
	OSErr err = 0;
...
OSErr TMover::DownItem(ListItem item)
{
	long i;
	OSErr err = 0;
...
OSErr TMover::MakeClone(TMover **clonePtrPtr)
{
...
OSErr TMover::BecomeClone(TMover *clone)
{
	OSErr err = 0;
...
OSErr TMover::CheckAndPassOnMessage(TModelMessage *message)
{	// JLM
...
Boolean TMover::FunctionEnabled(ListItem item, short buttonID)
{
	//long i;
