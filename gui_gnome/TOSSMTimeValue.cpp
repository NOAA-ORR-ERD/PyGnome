...
{//JLM
	long listLength;
	/*CHECK*/ listLength =  dynamic_cast<TOSSMTimeValue *>(this)->GetNumValues();
...
{//JLM
	...
	if( 0 <= n && n< TOSSMTimeValue::GetListLength())
	{
		...
		{
			switch(/*CHECK*/dynamic_cast<TOSSMTimeValue *>(this)->GetUserUnits())
			{
			...
			/*CHECK*/ConvertToUnits (dynamic_cast<TOSSMTimeValue *>(this)->GetUserUnits(), unitsStr);
		}
...
OSErr TOSSMTimeValue::Write(BFPB *bfpb)
{
	...
	if (timeValues) n = /*CHECK*/dynamic_cast<TOSSMTimeValue *>(this)->GetNumValues();
...
OSErr TOSSMTimeValue::ReadHydrologyHeader (char *path)
{
	OSErr	err = noErr;
	long	line = 0;
	char	strLine [512];
...
OSErr TOSSMTimeValue::ReadTimeValues (char *path, short format, short unitsIfKnownInAdvance)
{
	...
	/*CHECK*/ dynamic_cast<TOSSMTimeValue *>(this)->SetUserUnits(selectedUnits);
