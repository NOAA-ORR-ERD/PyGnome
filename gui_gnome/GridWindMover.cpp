...
{
	ListItem item = { dynamic_cast<GridWindMover *>(this), n, indent, 0 };
	long mode = model->GetModelMode();
	...
	{	
		switch(item.index)
		{
			...
			case I_GRIDWINDNAME:
				...
				WindSettingsDialog(dynamic_cast<GridWindMover *>(this), this -> moverMap,false,mapWindow,false);
				break;
			default:
				break;
		}
...
OSErr GridWindMover::DeleteItem(ListItem item)
{
	if (item.index == I_GRIDWINDNAME)
		return moverMap -> DropMover(dynamic_cast<GridWindMover *>(this));
