...
### Using conda package/environment management system ###

Download [miniconda](http://conda.pydata.org/miniconda.html) and install. Then follow steps below.

```
$ conda update conda  # to make sure you have latest conda
```

Updating conda should install `conda-env` to manage environments. Run `conda env` to check it is installed.
If usage info gets printed then it is installed else you may need to manually install by doing
`conda install conda-env`

If `conda develop` is not found, conda install it as `conda install conda-build`

Next run conda env create from py_gnome directory since this is where the environment.yml is located

```
$ cd py_gnome
$ conda env create            # create py_gnome environment with dependencies
$ source activate py_gnome    # activate environment
$ conda develop --build_ext . # builds py_gnome in develop mode
```

The last step builds the cython extensions and corrects the linkages for the libraries so at runtime
the libraries are loaded from the py_gnome environment.

To clean all autogenerated cython files and build files, invoke `python setup.py cleandev`
To uninstall py_gnome package, invoke `conda develop --uninstall .` (again done from py_gnome directory)

### Other resources - not using conda ###
