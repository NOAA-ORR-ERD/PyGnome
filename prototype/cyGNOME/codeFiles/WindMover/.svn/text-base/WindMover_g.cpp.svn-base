...
long WindMover_g::GetListLength()
{
...
ListItem WindMover_g::GetNthListItem(long n, short indent, short *style, char *text)
{
...
Boolean WindMover_g::ListClick(ListItem item, Boolean inBullet, Boolean doubleClick)
{
	OSErr err = 0;
	if (inBullet)
		switch (item.index) {
...
Boolean WindMover_g::FunctionEnabled(ListItem item, short buttonID)
{
	long i;
	switch (item.index) {
		case I_WINDNAME:
...
OSErr WindMover_g::SettingsItem(ListItem item)
{
...
OSErr WindMover_g::DeleteItem(ListItem item)
{
...
void WindMover_g::GetFileName(char* fileName) 
{ 	// JLM 7/29/98
...
OSErr WindMover_g::MakeClone(TClassID **clonePtrPtr)
{
	...
	{	// copy the fields
		if((*clonePtrPtr)->GetClassID() == this->GetClassID()) // safety check
		{
			...
			err =  Mover_g::MakeClone(clonePtrPtr);//  pass clone to base class
			if(!err) 
			{
				// we don't need to clone the uncertainty stuff
				// it can be recreated when the model is run
				if(this->timeDep)
					err = this->timeDep->MakeClone((TClassID**)&cloneP->timeDep);
...
OSErr WindMover_g::BecomeClone(TClassID *clone)
{
	OSErr err = 0;
	...
	if(clone)
	{
		if(clone->GetClassID() == this->GetClassID()) // safety check
		{
			...
			dynamic_cast<TWindMover *>(this)->Dispose(); // get rid of any memory we currently are using
			////////////////////
			// do the memory stuff first, in case it fails
			////////
			if(cloneP->timeDep)
				err = cloneP->timeDep->MakeClone((TClassID **)&this->timeDep);
			if(err) goto done;
			
			err =  Mover_g::BecomeClone(clone);//  pass clone to base class
			if(err) goto done;
	...
	if(err) dynamic_cast<TWindMover *>(this)->Dispose(); // don't leave ourselves in a weird state
	return err;
}

OSErr WindMover_g::CheckAndPassOnMessage(TModelMessage *message)
{	// JLM
...
void WindMover_g::DrawWindVector(Rect r, WorldRect view)
{
	Point		p;
	...
	for (i=0;i<istep;i++)
	{
		...
		err = dynamic_cast<TWindMover *>(this)->GetTimeValue (model->GetModelTime(), &velocity_orig);
...
OSErr WindMover_g::Write(BFPB *bfpb)
{
...
OSErr WindMover_g::Read(BFPB *bfpb)
{
	long version;
	ClassID id;
	Boolean haveTimeDep;
...
OSErr WindMover_g::ExportVariableWind(char* path)
{
	OSErr err = 0;
