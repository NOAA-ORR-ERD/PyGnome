...
OSErr ADCPMover::ComputeVelocityScale()
...
OSErr ADCPMover::AddUncertainty(long setIndex, long leIndex,VelocityRec *patVelocity,double timeStep,Boolean useEddyUncertainty)
{
...
OSErr ADCPMover::PrepareForModelStep()
{
	OSErr err =0;
...
void ADCPMover::ModelStepIsDone()
{
	memset(&fOptimize,0,sizeof(fOptimize));
}


WorldPoint3D ADCPMover::GetMove(Seconds timeStep,long setIndex,long leIndex,LERec *theLE,LETYPE leType)
{
...
VelocityRec ADCPMover::GetScaledPatValue(WorldPoint p,Boolean * useEddyUncertainty)
{	
...
VelocityRec ADCPMover::GetVelocityAtPoint(WorldPoint3D p)
...
VelocityRec ADCPMover::GetPatValue(WorldPoint p)
{
...
Boolean ADCPMover::VelocityStrAtPoint(WorldPoint3D wp, char *diagnosticStr)
{
...
OSErr ADCPMover::AddTimeDep(ADCPTimeValue *theTimeDep, short where)
{
	OSErr err = 0;
...
OSErr ADCPMover::DropTimeDep(ADCPTimeValue *theTimeDep)
{
	long i;
	OSErr err = 0;
...
ADCPTimeValue*	ADCPMover::AddADCP(OSErr *err)
{	// might send in path for first adcp
	*err = 0;
	...
	if (IsADCPFile(path))
	{
		timeValObj = new ADCPTimeValue(this);
...
OSErr ADCPMover::ReplaceMover()
{
	...
	if (mover)
	{
		...
		ADCPDialogNonPtrFields fields = GetADCPDialogNonPtrFields(this);
		SetADCPDialogNonPtrFields(mover,&fields);
		/*if(this->timeDep)
		{
			err = this->timeDep->MakeClone((TClassID**)&mover->timeDep);
...
OSErr ADCPMover::Read(BFPB *bfpb)
{
	...
	for (i = 0; i < numADCPs; i++)
	{
		...
		switch (id) {
			...
			case TYPE_ADCPTIMEVALUES: thisTimeDep = new ADCPTimeValue(this); break;
...
//					VLUpdate (&objects);
	...
	if (doubleClick && !inBullet)
	{
		switch(item.index)
		{
			...
			default:
				ADCPSettingsDialog (this, this -> moverMap, &timeFileChanged);
				return TRUE;
				break;
		}
	}
...
OSErr ADCPMover::TextRead(char *path)
{
	...
	if (IsADCPFile(path))
	{
		ADCPTimeValue *timeValObj = new ADCPTimeValue(this);
		//timeDep = new ADCPTimeValue(this);
