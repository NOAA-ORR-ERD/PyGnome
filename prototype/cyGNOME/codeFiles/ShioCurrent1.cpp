...
//
{
	...
	short	errorFlag=0,numOfConstituents=0,numOfYearCorrections=0;
	...
	if( (rotFlag==1) || (rotFlag==2) ){
		//
		// with new algorythm to determine flood/ebb
	...
	_HUnlock((Handle)answers->timeHdl); _HUnlock((Handle)answers->speedHdl);
...
// that are over 7.5 knots
{
	...
	short	numOfPoints,numOfMaxMins;
	...
	MaxMinHdl = answers->EbbFloodSpeedsHdl;
	CHdl = answers->speedHdl;
	uVelHdl = answers->uHdl;
	vVelHdl= answers->vHdl;
	...
	for (i=0;i<numOfPoints;i++){
		...
		if( v > 7.5) {
			dv = v - 7.5;
			vfix = .635 * dv;
			(*answers->speedHdl)[i] = sign * (v - vfix);
			ratio = (v-vfix)/v;
	 		(*answers->uHdl)[i] = (*answers->uHdl)[i] * ratio;
	 		(*answers->vHdl)[i] = (*answers->vHdl)[i] * ratio;
		}
	...
	for (i=0;i<numOfMaxMins; i++){
		...
		if(v>7.5){
			dv = v - 7.5;
			vfix = .635 * dv;
			(*answers->EbbFloodSpeedsHdl)[i] = sign * (v - vfix);
		}	
...
// **********************************************
short CheckCurrDir(CONSTITUENTPTR constituent,COMPCURRENTSPTR answers)
...
{	
	...
	theCurrentHdl	= answers->speedHdl;
	nFloodEbbs		= answers->numEbbFloods;
	maxMinHdl		= answers->EbbFloodSpeedsHdl;
	keyHdl			= answers->EbbFloodHdl;
	uHdl			= answers->uHdl;
	vHdl			= answers->vHdl;
...
//***********************************************
void CleanUpCompCurrents (COMPCURRENTSPTR CPtr)
{
	double		**hHdl,**HLHHdl;
	...
	CPtr->timeHdl			= nil;
	CPtr->speedHdl			= nil;
	CPtr->EbbFloodSpeedsHdl	= nil;
	CPtr->EbbFloodTimesHdl	= nil;
	CPtr->EbbFloodHdl		= nil;
...
short CurveFitOffsetCurve(COMPCURRENTSPTR answers,
							double startTime,
							double endTime,
							double timestep)   
{
...
// stuff ...
	...
	EXTFLAGHDL	MaxMinTimeHdl,TimeHdl;
	short		**MaxMinFlagHdl;
	...
	MaxMinHdl		= answers->EbbFloodSpeedsHdl;
	MaxMinTimeHdl	= answers->EbbFloodTimesHdl;
	MaxMinFlagHdl	= answers->EbbFloodHdl;
	ValHdl			= answers->speedHdl;
...
short DoOffSetCurrents (COMPCURRENTSPTR answers,  // Hdl to reference station heights
					CURRENTOFFSETPTR offset,      // Hdl to offset data
					double OldBeginHour,
					double OldEndHour,
					double timestep,
					short rotFlag)
...
void FixCEnds(COMPCURRENTSPTR answers,double beginTime, double endTime,short rotFlag)
...
//
{
	...
	EXTFLAGHDL theTimeHdl,EFTimesHdl;
	short numberOfPoints,i,startCross,endCross,numberOfEbbsFloods;
	...
	theCurrentHdl	= answers->speedHdl;
	theUHdl			= answers->uHdl;
	theVHdl			= answers->vHdl;
	...
	EFTimesHdl = answers->EbbFloodTimesHdl;
...
{
	double		**theCurrentHdl,**maxMinHdl;
	EXTFLAGHDL theTimeHdl,maxMinTHdl;
	short 		**keyHdl;
	short numberOfPoints,i,nFloodEbbs,j,flag;
	...
	nFloodEbbs		= answers->numEbbFloods;
	maxMinHdl		= answers->EbbFloodSpeedsHdl;
	maxMinTHdl		= answers->EbbFloodTimesHdl;
	keyHdl			= answers->EbbFloodHdl;
...
//*********************************************************************
short GetCDirec(CONSTITUENTPTR constituent, short *FDir, short *EDir)
{	
	short errorFlag,flood,ebb;
	
    // Assume space already aloocated
...
/*---------------------------------------------*/
double GetDatum(CONSTITUENTPTR constituent)
{	
	double datum;
...
/*---------------------------------------------*/
double GetDatum2(CONSTITUENTPTR constituent,short index)
