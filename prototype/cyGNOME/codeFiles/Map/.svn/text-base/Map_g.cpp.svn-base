...
OSErr Map_g::ReplaceMap() 
{
	printError("Button not implemented."); 
	return USERCANCEL;	// to return to dialog
}

OSErr Map_g::AddMover(TMover *theMover, short where)
{
	OSErr err = 0;
...
OSErr Map_g::DropMover(TMover *theMover)
{
	long 	i;
	OSErr	err = noErr;
...
OSErr Map_g::CheckAndPassOnMessage(TModelMessage *message)
{	// JLM
	long i, n;
	TMover *mover;
	OSErr err = 0;
				...
				else
				{
					if (IsNetCDFFile(netcdfFilePath,&gridType) || IsNetCDFPathsFile(netcdfFilePath, &isNetCDFPathsFile, fileNamesPath, &gridType))
					{
						...
						if (gridType == CURVILINEAR)
						{
							...
							TMap *newMap = 0;
							err = ((NetCDFWindMoverCurv*)newMover) -> TextRead(netcdfFilePath,&newMap);
						}
						else
						{
						...
						if (!err && isNetCDFPathsFile) /// JLM 5/3/10
						{
							...
							if(!err) err = newMover->SetInterval(errmsg);
						}
						///////////////////
						if(err) {
...
Boolean Map_g::IsDirty()
{
	long i, n;
	TMover *mover;
	Boolean	bIsDirty = false;
	
	if (ClassID_g::IsDirty ())
		bIsDirty = true;
	else if (moverList)
	{
		for (i = 0, n = moverList->GetItemCount() ; i < n ; i++)
...
void Map_g::Draw(Rect r, WorldRect view)
{
	long i, n;
	Rect m;
	TMover *mover;
...
Boolean Map_g::DrawingDependsOnTime(void)
{
	long i, m;
	CMyList *moverList;
	TMover *thisMover;
...
ListItem Map_g::GetNthListItem(long n, short indent, short *style, char *text)
{
	long i, m, count;
	TMover *mover;
...
Boolean Map_g::ListClick(ListItem item, Boolean inBullet, Boolean doubleClick)
{
	if (inBullet)
		switch (item.index) {
...
Boolean Map_g::FunctionEnabled(ListItem item, short buttonID)
{
	long i;
...
OSErr Map_g::UpItem(ListItem item)
{
	long i;
	OSErr err = 0;
...
OSErr Map_g::DownItem(ListItem item)
{
	long i;
	OSErr err = 0;
...
OSErr Map_g::AddItem(ListItem item)
{
	Boolean	timeFileChanged = false;
	short type, dItem;
	OSErr err = 0;
					...
					if (newMover)
					{
						switch (newMover->GetClassID()) 
						{
							...
							case TYPE_CATSMOVER3D:
								err = CATSSettingsDialog ((TCATSMover*)newMover, dynamic_cast<TMap *>(this), &timeFileChanged);
								break;
							case TYPE_ADCPMOVER:
								err = ADCPSettingsDialog ((ADCPMover*)newMover, dynamic_cast<TMap *>(this), &timeFileChanged);
								break;
				...
				{	
					...
					if (IsNetCDFFile(path,&gridType) || IsNetCDFPathsFile(path, &isNetCDFPathsFile, fileNamesPath, &gridType))
					{
						...
						if (gridType == CURVILINEAR)
						{
							...
							TMap *newMap = 0;
							err = ((NetCDFWindMoverCurv*)newMover) -> TextRead(path,&newMap);
						}
						else
						{
...
OSErr Map_g::SettingsItem(ListItem item)
{
	return TMapSettingsDialog(dynamic_cast<TMap *>(this));
}

OSErr Map_g::DeleteItem(ListItem item)
{
	if (item.index == I_MAPNAME)
