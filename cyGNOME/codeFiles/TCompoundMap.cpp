...
TCurrentMover* TCompoundMap::GetCompoundMover()
{
	...
	for (i = 0, d = this -> moverList -> GetItemCount (); i < d; i++)
	{
		...
		if(thisMover -> IAm(TYPE_COMPOUNDMOVER)) return (TCurrentMover*)thisMover;
	}
	return nil;
}
...
OSErr TCompoundMap::Read(BFPB *bfpb)
{
	...
	if (compoundMover) numMovers = ((TCompoundMover*)compoundMover)->moverList->GetItemCount();
	for (i = 0 ; i < numMaps ; i++) {
		if (err = ReadMacValue(bfpb,&id)) return err;
		switch (id) {
		...
		{
			((TCompoundMover*)compoundMover)->moverList->GetListItem((Ptr)&mover, i);
...
long TCompoundMap::PointOnWhichSeg(long point)	// This is really which boundary
{
...
Boolean TCompoundMap::ContiguousPoints(long p1, long p2)
{
...
long TCompoundMap::PointOnWhichSeg(long longVal, long latVal, long *startver, long *endver, float *distToSeg)
{
	long numSegs = GetNumBoundarySegs(), jseg;
...
TMover* TCompoundMap::GetMover(ClassID desiredClassID)
{
...
TTriGridVel* TCompoundMap::GetGrid(Boolean wantRefinedGrid)
{
	...
	if (mover)
	{
		return ((TCompoundMover *)mover) -> GetGrid(false);
	}
	else	
	{
	}
...
TTriGridVel3D* TCompoundMap::GetGrid3D(Boolean wantRefinedGrid)
{
	...
	if (mover)
	{
		return ((TCompoundMover *)mover) -> GetGrid3D(false);
	}
	else	
	{
	}
...
TTriGridVel3D* TCompoundMap::GetGrid3DFromMapIndex(long mapIndex)
{
	TTriGridVel3D* triGrid = 0;	
	TMover *mover = 0;
	...
	if (mover)
	{
		return ((TCompoundMover *)mover) -> GetGrid3DFromMoverIndex(mapIndex);
	}
	else	
	{
	}
...
TCurrentMover* TCompoundMap::Get3DCurrentMover()
{	
	TMover *thisMover = nil;
	long i,d;
	for (i = 0, d = this -> moverList -> GetItemCount (); i < d; i++)
	{
		...
		if (thisMover->IAm(TYPE_COMPOUNDMOVER))
			return ((TCompoundMover*)thisMover)->Get3DCurrentMover();	// which one to use?
	}
	return nil;
}

TCurrentMover* TCompoundMap::Get3DCurrentMoverFromIndex(long moverIndex)
{	
	TMover *thisMover = nil;
	long i,d;
	for (i = 0, d = this -> moverList -> GetItemCount (); i < d; i++)
	{
		this -> moverList -> GetListItem ((Ptr) &thisMover, i);
		if (thisMover->IAm(TYPE_COMPOUNDMOVER))
			return ((TCompoundMover*)thisMover)->Get3DCurrentMoverFromIndex(moverIndex);	// which one to use?
	}
	return nil;
}

LongPointHdl TCompoundMap::GetPointsHdl(Boolean useRefinedGrid)	// always false at this point...
{
	...
	mover = this->GetMover(TYPE_COMPOUNDMOVER);
	if (mover)
		ptsHdl = ((TCompoundMover *)mover)->GetPointsHdl();
	else
	{
	}
...
Boolean TCompoundMap::InVerticalMap(WorldPoint3D wp)
{
...
{	// here need to check by priority
	...
	NetCDFMover *mover = (NetCDFMover*)(Get3DCurrentMoverFromIndex(mapIndex));	// may not be netcdfmover?
	triGrid = GetGrid3DFromMapIndex(mapIndex);
	
	if (mover && mover->fVar.gridType==SIGMA_ROMS)
		return (double)((NetCDFMoverCurv*)mover)->GetTotalDepth(wp,-1);	// expand options here
...
/*float TCompoundMap::GetMaxDepth(void)
{	// 2D grid
	long i,numDepths;
...
float TCompoundMap::GetMaxDepth2(void)
...
WorldPoint3D TCompoundMap::TurnLEAlongShoreLine(WorldPoint3D waterPoint, WorldPoint3D beachedPoint, WorldPoint3D toPoint)
{
...
double TCompoundMap::DepthAtCentroid(long triNum)
{
	float depth1,depth2,depth3;
	double depthAtPoint;	
...
double TCompoundMap::DepthAtCentroidFromMapIndex(long triNum,long mapIndex)
{
	float depth1,depth2,depth3;
	double depthAtPoint;	
	long ptIndex1,ptIndex2,ptIndex3;
...
/*double TCompoundMap::GetBreakingWaveHeight(void)
{
...
long TCompoundMap::GetNumBoundarySegs(void)
{
	long i,numMaps;
	TMap *map = 0;
...
long TCompoundMap::GetNumPointsInBoundarySeg(long segno)
{
	long i,numMaps;
	TMap *map = 0;
...
long TCompoundMap::GetNumBoundaryPts(void)
{
	long numMaps;
	TMap *map = 0;
...
Boolean TCompoundMap::IsBoundaryPoint(long pt)
{
	return pt < GetNumBoundaryPts();
}

void TCompoundMap::FindStartEndSeg(long ptnum,long *startPt, long *endPt)
{
...
long TCompoundMap::NextPointOnSeg(long segno, long point)
{
...
long TCompoundMap::PrevPointOnSeg(long segno, long point)
{
	long incr = segno == 0? 1 : -1;
	long startno, endno;
...
/*Boolean TCompoundMap::MoreSegments(LONGH segh,long *startIndex, long *endIndex,long *curIndex)
{
...
void TCompoundMap::InitBoundaryIter(Boolean clockwise,long segno, long startno, long endno)
{
	theSegno = segno;
	theSegStart = startno;
	theSegEnd = endno;
...
Boolean TCompoundMap::MoreBoundarySegments(long *a,long *b)
{
	long j;
...
double TCompoundMap::PathLength(Boolean selectionDirection,long segNo, long startno, long endno)
{
	long p1,p2;
...
{	// new version combines triangle area and contours
	...
	if ((mover) && mover->IAm(TYPE_NETCDFMOVER)) {((NetCDFMover*)mover)->DrawContourScale(r,view);}
...
/*long TCompoundMap::WhichSelectedSegAmIIn(long index)
{
...
void TCompoundMap::DrawSegmentLabels(Rect r)
{
...
void TCompoundMap::DrawPointLabels(Rect r)
{
...
void TCompoundMap::DrawBoundaries(Rect r)
{
	long nSegs = GetNumBoundarySegs();	
...
void TCompoundMap::DrawBoundaries2(Rect r)
{
...
//OSErr TCompoundMap::CreateDepthSlice(TLEList *thisLEList, long triNum)	
OSErr TCompoundMap::CreateDepthSlice(long triNum, float **depthSlice)	
//OSErr TCompoundMap::CreateDepthSlice(long triNum, float *depthSlice)	
{
	LERec LE;
	LongPoint lp;
...
long TCompoundMap::CountLEsOnSelectedBeach()
{
...
/*void TCompoundMap::AddSegmentToSegHdl(long startno)
{
