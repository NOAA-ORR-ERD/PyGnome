...
/*		enum { KILOGRAMS = 1, METRICTONS, SHORTTONS,
    ...
    for(j = 0; j < c; j++) {
		...
		netStore->age[j] += (model->currentStep)*(model->GetTimeStep());
		switch(tLE->statusCode) {
			case OILSTAT_NOTRELEASED:
				netStore->flag[j] = 0;
				break;
...
// ++ Global Attributes: 
	...
	ncErr = nc_put_att_text(ncID, NC_GLOBAL, "creation_date", strlen(tStr), tStr);
	if(!CheckNC(ncErr)) return false;
	
	tStr = "GNOME version 1.3.12";
...
// Leaving definition mode.
	...
	{
		const int tRange[] = {0, 7};
		ncErr = nc_put_att_int(ncID, (*ncVarIDs)["Flag"], "valid_range", NC_BYTE, 2, tRange);
		if(!CheckNC(ncErr)) return false;

		const int tVals[] = { 0, 1, 2, 3, 4 };
		ncErr = nc_put_att_int(ncID, (*ncVarIDs)["Flag"], "flag_values", NC_BYTE, 5, tVals);
		if(!CheckNC(ncErr)) return false;
		
		tStr = "not_released in_water on_land off_maps evaporated";
