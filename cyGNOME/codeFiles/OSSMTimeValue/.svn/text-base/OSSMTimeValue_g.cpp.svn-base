...
OSErr OSSMTimeValue_g::MakeClone(TClassID **clonePtrPtr)
{
	...
	{	// copy the fields
		if((*clonePtrPtr)->GetClassID() == this->GetClassID()) // safety check
		{
			...
			err =  TimeValue_g::MakeClone(clonePtrPtr);//  pass clone to base class
			if(!err) 
			{
				if(this->timeValues)
				{
...
OSErr OSSMTimeValue_g::BecomeClone(TClassID *clone)
{
	OSErr err = 0;
