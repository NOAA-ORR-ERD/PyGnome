...
static TCompoundMap	*sSharedDialogCompoundMap = 0;
...
Boolean TCompoundMover::IAmA3DMover()
{
	long i,n;
	TMover *mover = 0;
...
OSErr TCompoundMover::AddUncertainty(long setIndex, long leIndex,VelocityRec *patVelocity,double timeStep)
{
...
OSErr TCompoundMover::SettingsItem(ListItem item)
{
	...
	switch (item.index) {
		default:	// need new dialog here
			return CompoundMoverSettingsDialog(this, this -> moverMap, &newMap);	// if new map is added ? 
	}
	return 0;
}
...
Boolean TCompoundMover::ListClick(ListItem item, Boolean inBullet, Boolean doubleClick)
{
	...
	if (doubleClick)
	{
		CompoundMoverSettingsDialog(this, this -> moverMap, &newMap);	// if new map is added ? 
	}	
...
void TCompoundMover::ModelStepIsDone()
{
	long i, n;
	TMover *mover;
...
OSErr TCompoundMover::PrepareForModelStep()
{
	char errmsg[256];
	OSErr err = 0;
...
WorldPoint3D TCompoundMover::GetMove (Seconds timeStep,long setIndex,long leIndex,LERec *theLE,LETYPE leType)
{
	double 		dLat, dLong;
...
Boolean TCompoundMover::VelocityStrAtPoint(WorldPoint3D wp, char *diagnosticStr)
{
...
LongPointHdl TCompoundMover::GetPointsHdl()
{
	long i,n;
	TMover *mover = 0;
	for (i = 0, n = moverList->GetItemCount() ; i < n ; i++) 
...
TTriGridVel* TCompoundMover::GetGrid(Boolean wantRefinedGrid)
{
	long i,n;
	TMover *mover = 0;
	...
	{	// movers should be listed in priority order - which one to use?
		...
		if (mover)
		{
			triGrid = (TTriGridVel*)(((NetCDFMover *)mover) -> fGrid);
...
TTriGridVel3D* TCompoundMover::GetGrid3D(Boolean wantRefinedGrid)
{
	long i,n;
	TMover *mover = 0;
...
TTriGridVel3D* TCompoundMover::GetGrid3DFromMoverIndex(long moverIndex)
{
	long i,n;
	TMover *mover = 0;
	TTriGridVel3D* triGrid = 0;	
...
TCurrentMover* TCompoundMover::Get3DCurrentMover()
{
...
TCurrentMover* TCompoundMover::Get3DCurrentMoverFromIndex(long moverIndex)
{
	TMover *thisMover = nil;
...
/////////////////////////////////////////////////
float TCompoundMover::GetArrowDepth()
{
	long i,n;
	float depth = 0.;
	TMover *mover = 0;
...
Boolean TCompoundMover::ShowDepthContourChecked()
{
	...
	{	// movers should be listed in priority order
		moverList->GetListItem((Ptr)&mover, i);
		//should have separate arrowDepth for combined mover ?, otherwise which one to use?
		if (mover->IAm(TYPE_NETCDFMOVER) && ((NetCDFMover*)mover)->bShowDepthContours == true) return true;
	}
	return false;
}
