...
built in.
Thus it's a nice target for running GNOME on your own system.
...
Anaconda will give you a wide variety of extra stuff,
including development tools, that are very useful for scientific computing.
It is a big install, though.
...
[binstar](http://binstar.org) is a web site where people can host packages
for download.
The way this is done is through binstar "channels", which can be thought of
simply as places on binstar where collections of packages are bundled together
...
So we have set up 
[our own binstar channel](https://conda.binstar.org/noaa-orr-erd)
where we put various packages needed for py_gnome.
But you don't need to access the web site to use it...conda can find
everything you need:

To install the binstar client:
...
But to get the whole setup, this file has a full dump of a conda environment
...
>The system compiler for OS-X is XCode.
...
>- Click the "Xcode" dropdown menu button in the top left of the screen
>near the Apple logo
>- Click "Preferences", then click "Downloads".
>- Command Line Tools should be one of the downloadable items,
>and there should be an install button for that item.  Click to install.
...
>_Note: if you are building on windows, the python package setuptools needs_
>_to be at version 6 or higher to properly query the compiler environment._


### Linux ###

>Linux uses the GNU gcc compiler.
>If it is not already installed on your system, use your system package
>manager to get it.
...
And it is probably best to build a "develop" target for your py_gnome package
if you plan on developing or debugging the py_gnome source code.

Building the "develop" target allows changes in the package python code
(or source control updates), to take place immediately.

Of course if you plan on simply using the package, you may certainly build with
the "install" target.  Just keep in mind that any updates to the project will
need to be rebuilt in order to take effect.
...
>As far as I know, the same linking issues don't exist,
>so you can build directly.
>There are a number of options for building:

>> `> python setup.py developall`

>builds everything; both gnome and the oil_library modules.
...
>cleans files generated by the build as well as files auto-generated by cython:

A sub-module of py_gnome that is also built with these steps is
the oil_library.
When initially built using developall, it builds a database of oil properties
for py_gnome (and others) to use.

If the oil database schema has been updated either manually or through a
`git pull` you will need to re-build the oil database with this command:
...
We have an extensive set of functional tests to make sure that py_gnome
is working properly.
...
Note that the tests will try to auto-download some data files.
If you are not on the internet, this will fail.  And of course if you have a slow connection, these files could take a while to download.
