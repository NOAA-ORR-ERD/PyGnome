...
def model(sample_model, request):
    ...
    This fixtures adds another spill (PointSourceRelease) and a netcdf outputter
    to the model. 
...
def test_read_standard_arrays(model):
    ...
    for file_ in (o_put.netcdf_filename, o_put._u_netcdf_filename):
        for step in range(model.num_time_steps):
            scp = model._cache.load_timestep(step)
            nc_data = gnome.netcdf_outputter.NetCDFOutput.read_data(file_, step)
            
            # check time
            assert scp.LE('current_time_stamp',uncertain) == nc_data['current_time_stamp'].item()
...
def test_read_all_arrays(model):
    ...
    for file_ in (o_put.netcdf_filename, o_put._u_netcdf_filename):
        for step in range(model.num_time_steps):
            scp = model._cache.load_timestep(step)
            nc_data = gnome.netcdf_outputter.NetCDFOutput.read_data(file_, step, all_data=True)
            ...
            for key in scp.LE_data:
                if key == 'current_time_stamp':
                    ...
                    assert np.all( scp.LE(key,uncertain)[:] == nc_data[key] )
