...
def gen_sinusoid(filename=None):
    ...
    #           arrow_length_ratio=0.2, color='darkblue', pivot='tail')
    # ax.quiver(x, y, vx, vy, color='darkblue', pivot='tail', angles='xy',
    #           scale=1.5, scale_units='xy', width=0.0025)
    # ax.plot(x[2], y[2])
    ...
    # ax.quiver(xc, yc, zc, vxc, vyc, vzc, length=0.3,
    #           arrow_length_ratio=0.2, color='darkred', pivot='tail')
    psi = {'p_grid': (xc, yc, zc),
           'p_vel': (vxc, vyc, vzc)}
    ...
    # ax.quiver(xu, yu, zu, vxu, vyu, vzu, length=0.3,
    #           arrow_length_ratio=0.2, color='indigo', pivot='tail')
    u = {'u_grid': (xu, yu, zu),
         'u_vel': (vzu, vxu, vzu)}
    ...
    # ax.quiver(xv, yv, zv, vxv, vyv, vzv, length=0.3,
    #           arrow_length_ratio=0.2, color='olive', pivot='tail')
    v = {'v_grid': (xv, yv, zv),
         'v_vel': (vyv, vzv, vzv)}
