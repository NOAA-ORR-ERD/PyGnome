...
from gnome.spill import Spill, FloatingSpill, \
    PointSourceSurfaceRelease, SpatialRelease, SubsurfaceSpill, \
...
def test_uncertain_copy():
    ...
    spill = PointSourceSurfaceRelease(
        num_elements=100,
...
# use units of m^3 for volume
                                        ...
                                        # volume = 0 for this case

@pytest.mark.parametrize(('sp_obj', 'num_elems'), [
    (Spill(), 0),
    (FloatingSpill(volume=1e-6, volume_units='m^3'), 0),
    (PointSourceSurfaceRelease(5, (0., 0., 0.), datetime.now(),
     volume=1e-5, volume_units='m^3'), 5),
    (SubsurfaceSpill(volume=1e-5, volume_units='m^3'), 0),
    (SubsurfaceRelease(5, (0., 0., 0.), datetime.now(), volume=1e-4,
     volume_units='m^3'), 5),
    (SpatialRelease((0., 0., 0.), datetime.now(), volume=1e-3,
     volume_units='m^3'), 5),
    ])
def test_create_new_elements(sp_obj, num_elems):
    ...
    for (name, array) in arrays.iteritems():
        ...
        if num_elems > 0:
            if name == 'mass':
                exp_mass = sp_obj.oil_props.get_density('g/cm^3') \
                    * sp_obj.get_volume('cm^3')
...
class Test_PointSourceSurfaceRelease(object):
    ...
    def test_init(self):
        ...
        assert np.alltrue(arrays['positions']
                          == sp.array_types['positions'].initial_value)
    ...
    def test_model_run_after_release(self):
        ...
        sp = PointSourceSurfaceRelease(num_elements=self.num_elements,
                start_position=self.start_position,
                release_time=self.release_time)
        data = sp.release_elements(self.release_time
                                   + timedelta(hours=1), time_step=30
                                   * 60)
        assert data is None
        ...
        data = sp.release_elements(self.release_time
                                   + timedelta(hours=2), time_step=30
                                   * 60)
        assert data is None
        ...
        sp.rewind()
        arrays = sp.release_elements(self.release_time, time_step=30
                * 60)
        assert arrays['positions'].shape == (10, 3)
    ...
    def test_cont_line_release1(self):
        ...
        sp = PointSourceSurfaceRelease(num_elements=11,
                start_position=(-128.0, 28.0, 0),
                release_time=self.release_time, end_position=(-129.0,
                29.0, 0), end_release_time=self.release_time
