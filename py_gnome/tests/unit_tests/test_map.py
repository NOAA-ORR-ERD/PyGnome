...
def test_in_water_resolution():
    ...
    msg = '''A particle positioned on a coastline segment must be moved
...
class Test_GnomeMap:
    def test_on_map(self):
        ...
        assert gmap.on_map((0.0, 0.0, 0.0)) is True

        # too big latitude
        print gmap.on_map((0.0, 91.0, 0.0))
        assert gmap.on_map((0.0, 91.0, 0.0)) is False

        # too small latitude
        assert gmap.on_map((0.0, -91.0, 0.0)) is False

        # too big langitude
        assert gmap.on_map((0.0, 361.0, 0.0)) is False
    ...
    def test_on_land(self):
        gmap = gnome.map.GnomeMap()
        assert gmap.on_land((18.0, -87.0, 0.0)) is False

    def test_in_water(self):
        gmap = gnome.map.GnomeMap()

        assert gmap.in_water((18.0, -87.0, 0.0))

        assert gmap.in_water((370.0, -87.0, 0.0)) is False
    ...
    def test_allowable_spill_position(self):
        ...
        assert gmap.allowable_spill_position((18.0, -87.0, 0.0)) is True

        assert gmap.allowable_spill_position((370.0, -87.0, 0.0)) is False
...
class Test_RasterMap():
    ...
    def test_on_map(self):
        gmap = RasterMap(refloat_halflife=6,  # hours
                         bitmap_array=self.raster,
                         map_bounds=((-50, -30), (-50, 30),
                                     (50, 30), (50, -30)),
                         projection=NoProjection(),
                         )
        assert gmap.on_map((0.0, 0.0, 0.0))

        assert gmap.on_map((55.0, 0.0, 0.0)) is False
    ...
    def test_on_land(self):
        ...
        print gmap.on_land((10, 6, 0.0))
        assert gmap.on_land((10, 6, 0.0))  # right in the middle

        print "testing a water point:"
        assert not gmap.on_land((19.0, 11.0, 0.0))
    ...
    def test_spillable_area(self):
        ...
        assert not gmap.allowable_spill_position((10, 6, 0.0))

        print "testing a water point:"
        assert gmap.allowable_spill_position((19.0, 11.0, 0.0))
    ...
    def test_spillable_area2(self):
        ...
        assert gmap.allowable_spill_position((11.0, 3.0, 0.0))
        assert gmap.allowable_spill_position((14.0, 9.0, 0.0))

        # in polygon, but on land:
        assert not gmap.allowable_spill_position((11.0, 6.0, 0.0))

        # outside polygon, on land:
        assert not gmap.allowable_spill_position((8.0, 6.0, 0.0))

        # outside polygon, off land:
        assert not gmap.allowable_spill_position((3.0, 3.0, 0.0))
...
class TestRefloat:
    ...
    map = RasterMap(refloat_halflife=time_step / 3600.0,  # hours
    ...
    def reset(self):
        ...
        self.map.refloat_halflife = self.time_step / 3600.0
    ...
    def test_refloat_some_onland(self):
        ...
        self.reset()
        self.spill['status_codes'][:] = oil_status.in_water
        self.map.refloat_halflife = 3 * self.time_step / 3600.0
        ...
        expected = round(1.0 - 0.5 \
                         ** (self.time_step \
                             / (self.map.refloat_halflife * 3600.0)), 2) \
                 * 100
...
def test_map_from_bna1():
    ...
    """
    # very course raster
    bna_map = MapFromBNA("sample_data/house.bna",
                          raster_size = 169,
                          )
    # top two corners should all be water (0)
    assert bna_map.bitmap[ 0, 0] == 0
    assert bna_map.bitmap[-1,0] == 0

    assert bna_map.bitmap[ 0, -1] == 1
    assert bna_map.bitmap[ -1, -1] == 1
...
class Test_MapfromBNA:
    ...
    def test_map_in_water2(self):
        ...
        InWater = (-126.971456, 47.935608, 0.0)
    ...
    def test_map_not_spillable2(self):
        ...
        point = (127.244752, 47.585072, 0.0)
    ...
    def test_map_not_spillable3(self):
        ...
        point = (127.643856, 47.999608, 0.0)
...
class Test_full_move:
    ...
    def test_on_map(self):
        ...
        assert gmap.on_map((100.0, 1.0, 0.0)) is False
        assert gmap.on_map((0.0, 1.0, 0.0))
    ...
    def test_land_cross(self):
        ...
        assert np.array_equal(spill['next_positions'][0],
                              (10.0, 5.0, 0.0))
    ...
    def test_some_cross_array(self):
        ...
        assert np.array_equal(spill['status_codes'][3:], (oil_status.on_land,
                                                          ))
    ...
    def test_outside_raster(self):
        ...
        spill = TestSpillContainer(4)
        spill['positions'] = np.array(((30.0, 5.0, 0.0),  # outside right
                                       (-5.0, 5.0, 0.0),  # outside left
                                       (5.0, -5.0, 0.0),  # outside top
                                       (-5.0, -5.0, 0.0),  # outside upper left
                                       ), dtype=np.float64)
