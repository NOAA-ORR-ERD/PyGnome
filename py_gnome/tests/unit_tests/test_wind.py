...
def test_init(wind_circ):
    ...
    gtime_val = wm.get_timeseries(format="uv",
                                  units='meter per second').view(dtype=np.recarray)
...
class TestWind:
    ...
    def test_set_timeseries(self, all_winds):
        ...
        assert np.allclose(wm.get_timeseries()['value'][:, 0],
                           x['value'][:, 0],
                           atol, rtol)  # only matches to 10^-14
    ...
    def test_get_timeseries_uv(self, all_winds):
        ...
        Initialize from timeseries and test the get_time_value method
        """
        gtime_val = all_winds['wind'].get_timeseries(format="uv").view(dtype=np.recarray)
    ...
    def test_get_timeseries_by_time_scalar(self, all_winds):
        ...
        get_rq = all_winds['wind'].get_timeseries(format="r-theta", datetime=dt).view(dtype=np.recarray)
        get_uv = all_winds['wind'].get_timeseries(format="uv", datetime=dt).view(dtype=np.recarray)
        ...
        print "-----------------"
        print "u-bounds: ({0:0.4f},{1:0.4f});\t computed-u: ({2:0.4f})".format(min(all_winds['uv'].value[:2, 0]),
                                                                               max(all_winds['uv'].value[:2, 0]),
                                                                               get_uv.value[0, 0])
        print "v-bounds: ({0:0.4f},{1:0.4f});\t computed-v: ({2:0.4f})".format(min(all_winds['uv'].value[:2, 1]),
        ...
        print "-----------------"
        print "r-bounds: ({0:0.4f},{1:0.4f});\t computed-r: ({2:0.4f})".format(min(all_winds['rq'].value[:2, 0]),
                                                                               max(all_winds['rq'].value[:2, 0]),
                                                                               get_rq.value[0, 0])
        print "q-bounds: ({0:0.4f},{1:0.4f});\t computed-q: ({2:0.4f})".format(min(all_winds['rq'].value[:2, 1]),
