...
class TestWind:
    ...
    def test_set_timeseries(self, all_winds):
        ...
        assert np.allclose( wm.get_timeseries()['value'][:,0], x['value'][:,0], atol, rtol) # only matches to 10^-14
    ...
    def test_get_timeseries_by_time(self, all_winds):
        ...
        """
        gtime_val = all_winds['wind'].get_timeseries(format="r-theta", datetime=all_winds['rq'].time).view(dtype=np.recarray)
        assert np.all(gtime_val.time == all_winds['rq'].time)
        assert np.allclose(gtime_val.value, all_winds['rq'].value, atol, rtol)
...
def test_constant_wind():
    ...
    """
    wind = environment.ConstantWind(10, 45, 'knots')
