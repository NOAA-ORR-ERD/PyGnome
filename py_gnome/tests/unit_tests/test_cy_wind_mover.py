...
from gnome.basic_types import spill_type, velocity_rec, \
    time_value_pair, world_point, world_point_type
...
class TestConstantWind:
    ...
    def test_move_value(self):
        ...
        xform = projections.FlatEarthProjection.meters_to_lonlat(delta,
                ref)
    ...
    def test_move(self):
        for x in range(0, 3):
            ...
            self.wm.prepare_for_model_step(self.cm.model_time
                    + vary_time, self.cm.time_step)
            self.wm.get_move(
                self.cm.model_time + vary_time,
                self.cm.time_step,
                self.cm.ref,
                self.delta,
                self.cm.windage,
