...
class TestWindMover:
    ...
    def test_get_move(self):
        ...
        self.wm.prepare_for_model_step(self.sc,
                                       self.time_step, self.model_time)

        for ix in range(2):
            curr_time = sec_to_date(date_to_sec(self.model_time) + (self.time_step * ix))
...
def test_new_from_dict():
    ...
    assert all([wm.__getattribute__(k) == wm2.__getattribute__(k)
                for k in WindMover.state.get_names('create')
                if k != 'wind_id' and k != 'obj_type'])
