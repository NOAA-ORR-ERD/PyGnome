...
input_file = os.path.join(datadir, 'MapBounds_2Spillable2Islands2Lakes.bna')
...
def test_render():
    ...
    """
    map_canvas.make_map(input_file, os.path.join(basedir, 'map_sample.png'))
...
def test_basemap_square():
    p1 = Polygon([[0, 45], [1, 45], [1, 46], [0, 46]],
                 metadata=('name', 'land', '1'))
