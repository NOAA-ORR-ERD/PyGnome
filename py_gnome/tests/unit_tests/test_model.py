...
def test_simple_run_with_map():
    ...
    spill = PointLineSource(num_elements=10, start_position=(0., 0., 0.),
...
def test_all_movers(start_time, release_delay, duration):
    ...
    release_time = start_time + timedelta(seconds=model.time_step
                                          * release_delay)
    model.spills += PointLineSource(num_elements=10, start_position=start_loc,
...
def test_linearity_of_wind_movers(wind_persist):
    ...
    elem_type = floating(windage_persist=wind_persist)
...
def test_model_release_after_start():
    ...
    model.spills += gnome.spill.PointLineSource(num_elements=4,
                                                start_position=(0, 0, 0),
                                                release_time=release_time)
...
def test_release_at_right_time():
    ...
    # before the run - no elements present
    #                  since data_arrays get defined after 1st step
