...
def test_exceptions():
    with pytest.raises(ValueError):
        transforms.r_theta_to_uv_wind(np.array([0, 0]))
        transforms.r_theta_to_uv_wind(np.array([-1, 0]))
        transforms.r_theta_to_uv_wind(np.array([1, -1]))
        transforms.r_theta_to_uv_wind(np.array([1, 361]))
...
def test_r_theta_to_uv_wind():
    uv_out = transforms.r_theta_to_uv_wind(rq)
    print uv_out
    print uv
...
def test_uv_to_r_theta_wind():
    rq_out = transforms.uv_to_r_theta_wind(uv)
    print rq_out
    print rq
...
def test_wind_inverse():
    ...
    rq_out = transforms.uv_to_r_theta_wind( transforms.r_theta_to_uv_wind(rq))
    print rq
    print rq_out
...
def test_r_theta_to_uv_current():
    uv_out = transforms.r_theta_to_uv_current(rq_c)
    assert np.all( uv_out == uv_c )
    
def test_uv_to_r_theta_current():
    rq_out = transforms.uv_to_r_theta_current(uv_c)
...
def test_current_inverse():
    ...
    """
    rq = np.array([(random.uniform(0,1), random.uniform(0,360))], dtype=np.float64)
    rq_out = transforms.uv_to_r_theta_current( transforms.r_theta_to_uv_current(rq))
    print rq
    print rq_out
    assert np.allclose(rq, rq_out, 1e-10, 1e-10)
