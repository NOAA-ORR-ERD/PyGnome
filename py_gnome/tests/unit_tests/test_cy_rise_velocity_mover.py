...
from gnome.cy_gnome import cy_rise_velocity_mover,cy_helpers
import cy_fixtures
...
def test_exceptions():
    ...
    with pytest.raises(ValueError):
        cy_rise_velocity_mover.CyRiseVelocityMover(water_density=0,water_viscosity=0)

    
class TestRiseVelocity():
    cm = cy_fixtures.CyTestMove()
    rv = cy_rise_velocity_mover.CyRiseVelocityMover(water_density=1020,water_viscosity=.000001)    
    density = np.zeros((cm.num_le,), dtype=np.double)    
    droplet_size = np.zeros((cm.num_le,), dtype=np.int32)    
    #rise_velocity = np.zeros((cm.num_le,), dtype=np.double)    #set these values and try with NaNs
    ...
    def test_move(self):
        ...
        print 
        #print  "rise_velocity  = {0:0.1f}".format(self.rise_velocity ) + " get_move output:"
        print  "rise_velocity  = " 
    ...
    def test_update_coef(self):
        """
        Test that the move is different from original move since rise velocity is different
