...
class CyTestMove(object):
    ...
    def __init__(self, num_le=4):
        ...
        self.ref = np.zeros((self.num_le, ),
                            dtype=basic_types.world_point)  # LEs - initial locations

        self.time_step = 60
        time = datetime(2012, 8, 20, 13)
        self.model_time = time_utils.date_to_sec(time)

        self.status = np.empty((self.num_le, ),
        ...
        self.delta = np.zeros((self.num_le, ),
                              dtype=basic_types.world_point)
        self.u_delta = np.zeros((self.num_le, ),
                                dtype=basic_types.world_point)
