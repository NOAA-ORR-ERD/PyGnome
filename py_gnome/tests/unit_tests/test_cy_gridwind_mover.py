...
class TestGridWindMover():
    ...
    def test_move_reg(self):
        ...
        time_grid_file = os.path.join(winds_dir, 'test_wind.cdf')
        ...
        np.testing.assert_allclose(self.cm.delta['lat'], actual['lat'],
                                   tol, tol,
                                   msg.format("test_wind.cdf", tol), 0)
    ...
    def test_move_curv(self):
        ...
        time_grid_file = os.path.join(winds_dir, 'WindSpeedDirSubset.nc')
        topology_file = os.path.join(winds_dir, 'WindSpeedDirSubsetTop.dat')
        ...
        np.testing.assert_allclose(self.cm.delta['lat'], actual['lat'],
                                   tol, tol,
                                   msg.format("WindSpeedDirSubset.nc", tol), 0)
        np.testing.assert_allclose(self.cm.delta['long'], actual['long'],
                                   tol, tol,
                                   msg.format("WindSpeedDirSubset.nc", tol), 0)
...
#                                    msg.format("HiROMS", tol), 0)
    ...
    def test_move_gridwindtime(self):
        ...
        time_grid_file = os.path.join(winds_dir, 'gridwind_ts.wnd')
        topology_file = r""  # will want a null default
