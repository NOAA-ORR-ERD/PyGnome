...
from ..conftest import (test_oil, sample_model_weathering2, sample_model)
...
class ROCTests:
    @classmethod
    def mk_objs(cls):
        model = sample_model_weathering2(sample_model(), test_oil, 333.0)
...
class TestROCBurn(ROCTests):
    ...
    def test_prepare_for_model_run(self):
        ...
        assert self.burn._offset_time == 0.24675
        assert round(self.burn._boom_capacity) == 477
        assert len(self.sc.report[self.burn.id]) == 0
