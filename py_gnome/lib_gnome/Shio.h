...
#pragma options align=mac68k
		...
		typedef struct constiuentstruct2
		{
			CONTROLVAR		DatumControls;
			float			**H;
			float			**kPrime;
		}CONSTITUENT2,*CONSTITUENTPTR2, **CONSTITUENTHDL2;
...
{
	long			nPts;
	EXTFLAG			*time;            	// flag = 0 means plot
	double			*height;
...
typedef struct currentcompstruct
{
	long			nPts;
	EXTFLAG			*time;				// flag = 0 means plot
	double			*speed;				// vel. vector magnitude (- for ebb)
...
/*---------------------------------------------*/
typedef struct yeardatstruct2
{
	short	numElements;
	double	*XODE;
	double	*VPU;

} YEARDATA2,*YEARDATAPTR2, **YEARDATAHDL2;
...
short DoOffSetCorrection (COMPHEIGHTS *answers,  // Ptr to reference station heights
				       	  HEIGHTOFFSET *htOffset); // offset data

void DumpAnswers(COMPHEIGHTS *answers, double *julianoffset);
...
void FixHEnds(COMPHEIGHTS *answers,    // answers
...
short GetWeights(double t,                 // the time to interpolate at hrs
				EXTFLAG *TArray,           // the high low array of time in hrs
...
short CheckCurrDir(CONSTITUENT *constituent,  // flips answers if direction wrong
                  COMPCURRENTS *answers);

void CleanUpCompCurrents (COMPCURRENTS *CHdl);        // Hdl to answers

void CompCErrors(short errorNum,                       // error number
				 char *errStr);                        // error string returned

short CurveFitOffsetCurve(COMPCURRENTS *AnswerHdl,
							double startTime,
							double endTime,
							double timeStep);
...
void FixCEnds(COMPCURRENTS *answers,  // Answers
...
short GetCDirec(CONSTITUENT *constituent, // Constituent data handle
...
short GetSlackRatio(double t,                        // Time in hours
				    ...
				    EXTFLAG *EbbFloodTimesPtr,       // Time array of max mins
...
short GetVelDir(CONSTITUENT *constituent,  // return 1 if flood, -1 if ebb
                double u,
				double v);
