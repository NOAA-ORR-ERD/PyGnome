...
OSErr GridCurMover::SettingsDialog()
{
	...
	sGridCurDialogMover = this; // should pass in what is needed only
...
OSErr GridCurMover::AddUncertainty(long setIndex, long leIndex,VelocityRec *velocity,double timeStep,Boolean useEddyUncertainty)
{
...
OSErr GridCurMover::PrepareForModelStep()
{
...
void GridCurMover::ModelStepIsDone()
{
...
WorldPoint3D GridCurMover::GetMove(Seconds timeStep,long setIndex,long leIndex,LERec *theLE,LETYPE leType)
{
...
VelocityRec GridCurMover::GetScaledPatValue(WorldPoint p,Boolean * useEddyUncertainty)
{
	VelocityRec v = {0,0};
...
VelocityRec GridCurMover::GetPatValue(WorldPoint p)
{
	VelocityRec v = {0,0};
...
long GridCurMover::GetVelocityIndex(WorldPoint p) 
{
	long rowNum, colNum;
...
double GridCurMover::GetStartUVelocity(long index)
{	// 
	double u = 0;
...
double GridCurMover::GetEndUVelocity(long index)
{
	double u = 0;
...
double GridCurMover::GetStartVVelocity(long index)
{
	double v = 0;
	if (fStartData.dataHdl && index >= 0)
...
double GridCurMover::GetEndVVelocity(long index)
{
	double v = 0;
	if (fEndData.dataHdl && index >= 0)
...
OSErr GridCurMover::GetStartTime(Seconds *startTime)
{
	OSErr err = 0;
	*startTime = 0;
...
OSErr GridCurMover::GetEndTime(Seconds *endTime)
{
	OSErr err = 0;
	*endTime = 0;
...
Boolean GridCurMover::VelocityStrAtPoint(WorldPoint3D wp, char *diagnosticStr)
{
	char uStr[32],sStr[32];
	double lengthU, lengthS;
	VelocityRec velocity = {0.,0.};
	OSErr err = 0;
	long timeDataInterval,numTimesInFile = GetNumTimesInFile();
	Boolean intervalLoaded = this -> CheckInterval(timeDataInterval);
