...
Boolean TClassID::GetSelectedListItem(ListItem *item)
...
Boolean TClassID::SelectedListItemIsMine(void)
{	// returns TRUE if the selected item belongs to this object
	ListItem localItem;
	Boolean itemSelected = SelectedListItem(&localItem);
...
Boolean TClassID::IAmEditableInMapDrawingRect(void)
{	
	return FALSE;
}

Boolean TClassID::IAmCurrentlyEditableInMapDrawingRect(void)
{	
	return FALSE;
}

Boolean TClassID::UserIsEditingMeInMapDrawingRect(void)
{	
	return FALSE;
}

void TClassID::StartEditingInMapDrawingRect(void)
{	
	// ignore them
}

OSErr TClassID::StopEditingInMapDrawingRect(Boolean *deleteMe)
{	
	*deleteMe = FALSE;
	return 0;
}
...
OSErr TClassID::BecomeClone(TClassID *clone)
{
	OSErr err = 0;
...
void TClassID::SetClassName (char *newName)
{
...
OSErr TClassID::CheckAndPassOnMessage(TModelMessage *message)
{	// JLM
...
ListItem TClassID::GetNthListItem(long n, short indent, short *style, char *text)
{
...
Boolean TClassID::ListClick(ListItem item, Boolean inBullet, Boolean doubleClick)
{
	//	if (item.index % 10 == 0)
