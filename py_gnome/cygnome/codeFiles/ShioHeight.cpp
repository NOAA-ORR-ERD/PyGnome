...
//
{
	...
	short		numOfConstituents=0,errorFlag=0;
	short		beginDay=0,beginMonth=0,beginYear=0,endDay=0,endMonth=0,endYear=0;


	/* Varialbles to overlay reference curve */
	EXTFLAG		**reftimeHdl=nil;
	double		**refheightHdl=nil;
	...
	if(constituent->kPrime) numOfConstituents = (short)( (long)_GetHandleSize((Handle)constituent->kPrime) / (long)sizeof(float) );
	...
	if( gOverlayReferenceCurve ){
		...
		numpnts = (long)( (long)_GetHandleSize((Handle)(answers->timeHdl)) / (long)sizeof(EXTFLAG) );
		reftimeHdl=(EXTFLAG **)_NewHandleClear( numpnts * sizeof(EXTFLAG) );
		errorFlag = (short)_MemError(); if( errorFlag != 0 ){ goto Error; }
		...
		refheightHdl=(double **)_NewHandleClear( _GetHandleSize((Handle)(answers->heightHdl)) );
		errorFlag = (short)_MemError(); if( errorFlag != 0 ){ goto Error; }
	...
	***********************************************************************/
	...
	if( gOverlayReferenceCurve && reftimeHdl && refheightHdl ){
		// Append time data 
		size1=_GetHandleSize((Handle)reftimeHdl);
		size2=_GetHandleSize((Handle)answers->timeHdl);

		_HUnlock((Handle)answers->timeHdl); _HUnlock((Handle)reftimeHdl);

		_SetHandleSize((Handle)answers->timeHdl,(size1+size2));
		errorFlag=(short)_MemError(); if(errorFlag!=0){ goto Error; }

		_HLock((Handle)answers->timeHdl); _HLock((Handle)reftimeHdl);
		...
		// Append height data 
		size1=_GetHandleSize((Handle)refheightHdl);
		size2=_GetHandleSize((Handle)answers->heightHdl);

		_HUnlock((Handle)answers->heightHdl); _HUnlock((Handle)refheightHdl);

		_SetHandleSize((Handle)answers->heightHdl,(size1+size2));
		errorFlag=(short)_MemError(); if(errorFlag!=0){ goto Error; }

		_HLock((Handle)answers->heightHdl); _HLock((Handle)refheightHdl);
...
{
	double		**hHdl,**HLHHdl;
	EXTFLAGHDL	tHdl,HLTHdl;
	short		**HLHdl;
	
	tHdl	= cPtr->timeHdl;
	hHdl	= cPtr->heightHdl;
	HLHHdl	= cPtr->HighLowHeightsHdl;
	...
	cPtr->timeHdl			= 0;
	cPtr->heightHdl			= 0;
	cPtr->HighLowHeightsHdl	= 0;
	cPtr->HighLowTimesHdl	= 0;
	cPtr->HighLowHdl			= 0;
...
short DoOffSetCorrection (COMPHEIGHTSPTR answers,  // Hdl to reference station heights
					HEIGHTOFFSET htOffset)  // Hdl to offset data

{
	short		NoOfHighsLows,numOfPoints,i,index,errorFlag,flag;
	...
	double		w1,w2;
	double		**HLHeightHdl,**ValHdl;
	EXTFLAGHDL	HLTimeHdl,TimeHdl;
	short		**HLFlagHdl;
	double		*HArrayPtr;
	...
	HLHeightHdl	= answers->HighLowHeightsHdl;
	HLTimeHdl	= answers->HighLowTimesHdl;
	HLFlagHdl	= answers->HighLowHdl;
	ValHdl		= answers->heightHdl;
	TimeHdl		= answers->timeHdl;
	...
	/*HArrayPtr = (double *)_NewPtrClear(sizeof(double)*NoOfHighsLows);
	if(HArrayPtr==0){
		errorFlag=20;
		goto Error;
	}*/
	...
	/*TArrayPtr = (EXTFLAGPTR)_NewPtrClear(sizeof(EXTFLAG)*(NoOfHighsLows+2) );
	if(TArrayPtr==0){
		errorFlag=21;
		goto Error;
	}*/
...
Error:
	//if(HArrayPtr) _DisposePtr((Ptr)HArrayPtr);
...

{
	...
	tHdl = answers->HighLowHdl;
	THdl = answers->timeHdl;
	HHdl = answers->heightHdl;
	nPts = answers->nPts;
...
// value and returns it and its time.
{
	double timeSpan,t,h;
	double t0,t1,h0,h1;
	short low=0,findFlag,numOfSteps,i;
...
void FixHEnds(COMPHEIGHTSPTR answers,double beginTime, double endTime)
...
{
	double **theHeightHdl;
	EXTFLAGHDL theTimeHdl,theHLTimeHdl;
	short numberOfPoints,i,startCross,endCross,numberOfHighLows;
	double w1,w2,dt,dt1;
	double firstPointValue,lastPointValue;
	
	numberOfPoints	= answers->nPts;
	theTimeHdl		= answers->timeHdl;
	theHeightHdl	= answers->heightHdl;
	
	numberOfHighLows = answers->numHighLows;
	theHLTimeHdl    = answers->HighLowTimesHdl;
...

{
	...
	double		**HHdl=nil,**HLHHdl=nil;
	EXTFLAG		**THdl=nil,**HLTHdl=nil;
	long		maxPeaks=0, NumOfSteps=0,pcnt=0;
	short		**tHdl=nil,i=0,HighLowCount=0,errorFlag=0;
	Boolean		stop=false;
	OSErr 		err = 0;
	...
	for (i=0; i<numOfConstituents; i++){
		AMPAPtr[i]	= ( *constituent->H)[i] * (double)((*YHdl)[i]).XODE ;
		epochPtr[i]	= ( (double)((*YHdl)[i]).VPU - (double)(*constituent->kPrime)[i] );
	}
	...
	HHdl = (double **)_NewHandleClear( NumOfSteps*sizeof(double) ); errorFlag=(short)_MemError();
	if( errorFlag != 0 ){
		errorFlag=11;
		goto Error;
	}
	...
	HLHHdl = (double **)_NewHandleClear( maxPeaks*sizeof(double) ); errorFlag=(short)_MemError();
	if( errorFlag != 0 ){
		errorFlag=13;
		goto Error;
	}
	...
	HLTHdl = (EXTFLAGHDL)_NewHandleClear( (maxPeaks + 2)*sizeof(EXTFLAG) ); errorFlag=(short)_MemError();
	if( errorFlag != 0 ){
		errorFlag=14;
		goto Error;
	}
	...
	tHdl = (short **)_NewHandleClear( maxPeaks*sizeof(short) ); errorFlag=(short)_MemError();
	if( errorFlag != 0 ){
		errorFlag=15;
		goto Error;
	}
	...
	answers->heightHdl			= HHdl;
	answers->numHighLows			= HighLowCount;
	answers->HighLowHeightsHdl	= HLHHdl;
	answers->HighLowTimesHdl		= HLTHdl;
	answers->HighLowHdl			= tHdl;
...
{
	YEARDATAHDL		YHdl=nil;
	short			numYearStuff=0,i=0,errorFlag=0;


	YHdl=(YEARDATAHDL)GetResource('YEAR',(long)year);
	if(YHdl==nil)
	...
	numYearStuff = ( _GetPtrSize((Ptr)XODE)/sizeof(double) );
	
	for (i=0;i<numYearStuff;i++)

	{
		XODE[i]	=(double)(((*YHdl)[i]).XODE);
		VPU[i]	=(double)(((*YHdl)[i]).VPU);
	}

Error:
	if(YHdl){ ReleaseResource((Handle)YHdl); }
...
// function to reset time arrays to start from midnight of first day
{
	...
	theTimeHdl = answers->HighLowTimesHdl;
