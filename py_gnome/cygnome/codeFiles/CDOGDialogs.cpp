				...
				CDOGLEList *thisCDOGLEList = GetCDOGSpill();
					if (thisCDOGLEList)
					{
						...
						else if (thisCDOGLEList->GetMethodOfDeterminingHydrodynamics()==2)	// data from netCDF model
						{
							...
							if (possible3DMover->IAm(TYPE_NETCDFMOVER)) numDepthLevels = ((NetCDFMover*)possible3DMover)->GetNumDepthLevels();
...
OSErr CDOGLEList::ExportProfilesAsNetCDF(TCurrentMover *hydrodynamicData)
{
	...
	if (hydrodynamicData->IAm(TYPE_NETCDFMOVER)) numTimes = ((NetCDFMover*)hydrodynamicData)->GetNumTimesInFile();
	else return -1;
	if (numTimes < 1) return -1;

	sigmaLength = ((NetCDFMover*)hydrodynamicData)->GetNumDepthLevels();
	timeValInSecs = ((NetCDFMover*)hydrodynamicData)->GetTimeValue(numTimes-1);
  ...
  // status = nc_def_var (ncid, "reftime", NC_CHAR, 1, dimid, &reftime_id);
	...
	{
		timeIndex = k;
		if (hydrodynamicData->IAm(TYPE_NETCDFMOVER)) err = ((NetCDFMover*)hydrodynamicData)->GetDepthProfileAtPoint(fSetSummary.startRelPos,timeIndex,&profilesH);
		...
		timeValInSecs = ((NetCDFMover*)hydrodynamicData)->GetTimeValue(timeIndex);
