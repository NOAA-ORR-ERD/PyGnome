...
/*OSErr TCATSMover3D::ComputeVelocityScale()
...
OSErr TCATSMover3D::AddUncertainty(long setIndex, long leIndex,VelocityRec *patVelocity,double timeStep,Boolean useEddyUncertainty)
{
...
OSErr TCATSMover3D::PrepareForModelStep()
{
	OSErr err =0;
	
	TCurrentMover::PrepareForModelStep(); // note: this calls UpdateUncertainty()
...
/*void TCATSMover3D::ModelStepIsDone()
{
	memset(&fOptimize,0,sizeof(fOptimize));
}


WorldPoint3D TCATSMover3D::GetMove(Seconds timeStep,long setIndex,long leIndex,LERec *theLE,LETYPE leType)
{
...
VelocityRec TCATSMover3D::GetScaledPatValue(WorldPoint p,Boolean * useEddyUncertainty)
{
...
VelocityRec TCATSMover3D::GetPatValue(WorldPoint p)
{
	return fGrid->GetPatValue(p);	// don't use refined grid for movement
}

VelocityRec TCATSMover3D::GetSmoothVelocity (WorldPoint p)
{
	return fGrid->GetSmoothVelocity(p); // don't use refined grid for movement
}
*/
Boolean TCATSMover3D::VelocityStrAtPoint(WorldPoint3D wp, char *diagnosticStr)
{
...
/*void TCATSMover3D::DeleteTimeDep () 
{
	if (timeDep)
	{
		timeDep -> Dispose ();
...
//LongPointHdl TCATSMover3D::GetPointsHdl(Boolean useRefinedGrid)
LongPointHdl TCATSMover3D::GetPointsHdl()
{
