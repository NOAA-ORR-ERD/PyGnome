...
OSErr TOSSMTimeValue::MakeClone(TClassID **clonePtrPtr)
{
	...
	{	// copy the fields
		if((*clonePtrPtr)->GetClassID() == this->GetClassID()) // safety check
		{
			...
			err =  TTimeValue::MakeClone(clonePtrPtr);//  pass clone to base class
			if(!err) 
			{
				if(this->timeValues)
				{
...
OSErr TOSSMTimeValue::BecomeClone(TClassID *clone)
{
	OSErr err = 0;
