...
OSErr TriCurMover::AddUncertainty(long setIndex, long leIndex,VelocityRec *velocity,double timeStep,Boolean useEddyUncertainty)
{
...
OSErr TriCurMover::PrepareForModelStep()
{
...
void TriCurMover::ModelStepIsDone()
{
	fIsOptimizedForStep = false;
}

/*OSErr TriCurMover::CheckAndScanFile(char *errmsg)
{
...
Boolean TriCurMover::CheckInterval(long &timeDataInterval)
{
...
void TriCurMover::DisposeLoadedData(LoadedData *dataPtr)
{
	if(dataPtr -> dataHdl) DisposeHandle((Handle) dataPtr -> dataHdl);
	ClearLoadedData(dataPtr);
}

void TriCurMover::ClearLoadedData(LoadedData *dataPtr)
{
	dataPtr -> dataHdl = 0;
	dataPtr -> timeIndex = UNASSIGNEDINDEX;
}

long TriCurMover::GetNumTimesInFile()
{
	long numTimes = 0;
...
long TriCurMover::GetNumDepths(void)
{
	long numDepths = 0;
...
LongPointHdl TriCurMover::GetPointsHdl()
{
	TTriGridVel* triGrid = (TTriGridVel*)fGrid;
	return triGrid -> GetPointsHdl();
}

TopologyHdl TriCurMover::GetTopologyHdl()
{
	TTriGridVel* triGrid = (TTriGridVel*)fGrid;
	return triGrid -> GetTopologyHdl();
}

long TriCurMover::WhatTriAmIIn(WorldPoint wp)
{
	LongPoint lp;
	TTriGridVel* triGrid = (TTriGridVel*)fGrid;
...
OSErr TriCurMover::SetInterval(char *errmsg)
{
...
WorldPoint3D TriCurMover::GetMove(Seconds timeStep,long setIndex,long leIndex,LERec *theLE,LETYPE leType)
{
...
VelocityRec TriCurMover::GetScaledPatValue(WorldPoint p,Boolean * useEddyUncertainty)
{
	VelocityRec v = {0,0};
...
VelocityRec TriCurMover::GetPatValue(WorldPoint p)
{
	VelocityRec v = {0,0};
...
Boolean TriCurMover::VelocityStrAtPoint(WorldPoint3D wp, char *diagnosticStr)
{
...
OSErr TriCurMover::GetTriangleCentroid(long trinum, LongPoint *p)
{	
	long x[3],y[3];
...
float TriCurMover::GetMaxDepth(void)
{
	long i,numDepths;
...
void TriCurMover::GetDepthIndices(long triIndex, float depthAtPoint, long *depthIndex1, long *depthIndex2)
{
...
/////////////////////////////////////////////////////////////////
OSErr TriCurMover::CalculateVerticalGrid(LongPointHdl ptsH, FLOATH totalDepthH, TopologyHdl topH, long numTri, FLOATH sigmaLevelsH, long numSigmaLevels) 
{
...
OSErr TriCurMover::TextRead(char *path, TMap **newMap) 
{
	...
	else
	{
		...
		if (fVar.gridType == TWO_D) 
		{
			err = CalculateVerticalGrid(pts,totalDepthH,topo,numTopoPoints,sigmaLevelsH,numLevels);
			if (err) goto done;
		}
...
OSErr TriCurMover::ReadTimeData(long index,VelocityFH *velocityH, char* errmsg) 
{
	char s[256], path[256]; 
	long i,j,line = 0;
