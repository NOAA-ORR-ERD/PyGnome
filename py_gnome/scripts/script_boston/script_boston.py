...
def make_model(images_dir=os.path.join(base_dir, 'images')):
    ...
    gnome_map = gnome.map.MapFromBNA(mapfile, refloat_halflife=1)  # hours
    ...
    model = gnome.model.Model(time_step=900, start_time=start_time,
                              duration=timedelta(days=1),
                              map=gnome_map, uncertain=False)  # 15 minutes in seconds
