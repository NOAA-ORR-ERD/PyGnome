...
# This is the old file import that does not have the working TAMOC items
...
def make_model():
    ...
    oil_file = os.path.join(base_dir, 
        'Input/light-louisianna-sweet-bp_AD01554.json')
    subs = GnomeOil(filename=oil_file)

    # Add a spill object
    print('\n-- Adding a Point Spill           --')
    end_release_time = model.start_time + gs.hours(12)
    point_source = ts.TamocSpill(
    ...
    print('\n-- Adding Currents                --')
    current = gs.SteadyUniformCurrent(
        speed=0.1,
        direction=90.,
        units='m/s'
    )
