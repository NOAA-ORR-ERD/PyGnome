...
class remake_oil_db(develop):
    ...
    def run(self):
        '''
        just extend develop mode - no need to call base class run since we're
        just invoking oil_libraries setup file
        install oil lib - define custom command for setup.py to reinstall
        oil_library from py_gnome's setup.py
        '''
        build_oil_lib('remake_oil_db')


class developall(develop):
    description = "build py_gnome and oil_library in 'develop' mode"
