...
from .movers cimport IceWindMover_c, GridWindMover_c, WindMover_c

from gnome import basic_types
from gnome.cy_gnome.cy_gridwind_mover cimport CyGridWindMover
...
cdef extern from *:
    ...
    IceWindMover_c* dc_mover_to_im "dynamic_cast<IceWindMover_c *>" (Mover_c *) except NULL
    WindMover_c* dc_im_mover_to_cm "dynamic_cast<IceWindMover_c *>" (Mover_c *) except NULL
    GridWindMover_c* dc_im_mover_to_gcm "dynamic_cast<IceWindMover_c *>" (Mover_c *) except NULL


cdef class CyIceWindMover(CyGridWindMover):

    cdef IceWindMover_c *grid_wind_ice

    def __cinit__(self):
        self.mover = new IceWindMover_c()
        #self.grid_wind_ice = dynamic_cast_ptr(self.mover)
        self.grid_wind_ice = dc_mover_to_im(self.mover)
        self.wind = dc_im_mover_to_cm(self.mover)
        self.grid_wind = dc_im_mover_to_gcm(self.mover)

    def __dealloc__(self):
        del self.mover
        self.mover = NULL
        self.grid_wind_ice = NULL
        self.wind = NULL
        self.grid_wind = NULL
    ...
    def text_read(self, time_grid_file, topology_file=None):
        ...
        if topology_file is None:
            err = self.grid_wind_ice.TextRead(time_grid, '')
        else:
            topology = filename_as_bytes(topology_file)
            err = self.grid_wind_ice.TextRead(time_grid, topology)
        ...
        if err != 0:
            ...
            """
            raise OSError("IceWindMover_c.TextRead returned an error.")
...
#         pts_hdl = self.grid_wind_ice.GetPointsHdl()
...
#
#         # allocate memory and copy it over
#         pts_hdl = self.grid_wind_ice.GetTriangleCenters()
#         sz = _GetHandleSize(<Handle>pts_hdl)
#
#         # will this always work?
...
#         top_hdl = self.grid_wind_ice.GetTopologyHdl()
...
#         """
#         num_tri = self.grid_wind_ice.GetNumTriangles()
#
#         return num_tri
#
    ...
    def get_ice_fields(self, Seconds model_time,
        ...
        err = self.grid_wind_ice.GetIceFields(model_time,
        ...
        if err != 0:
            ...
            are defined and enumerated
            """
            raise OSError("IceWindMover_c.GetIceFields returned an error.")
    ...
    def get_ice_velocities(self, Seconds model_time,
        ...
        err = self.grid_wind_ice.GetIceVelocities(model_time, &vels[0])
        if err != 0:
            """
            For now just raise an OSError - until the types of possible errors
            are defined and enumerated
            """
            raise OSError("IceWindMover_c.GetIceVelocities returned an error.")
    ...
    def get_movement_velocities(self, Seconds model_time,
        ...
        err = self.grid_wind_ice.GetMovementVelocities(model_time, &vels[0])
        if err != 0:
            """
            For now just raise an OSError - until the types of possible errors
            are defined and enumerated
            """
            raise OSError('IceWindMover_c.GetMovementVelocities '
                          'returned an error.')
