...
cdef class CyRiseVelocityMover(cy_mover.CyMover):
    ...
    def get_move(self, 
        ...
        :param rise_velocity: rise_velocity to be applied to each particle (if not NaN)
        :param density: density used to calculate rise_velocity if necessary
        :param droplet_size: droplet_size used to calculate rise_velocity if necessary
        ...
        if err == 1:
            raise ValueError("Make sure numpy arrays for ref_points, delta and rise_velocity are defined")
