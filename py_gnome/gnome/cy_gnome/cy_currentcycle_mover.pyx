...
cdef class CyCurrentCycleMover(cy_mover.CyMover):
...
# 
    ...
    def __repr__(self):
        ...
        """
        info = "CyCurrentCycleMover(uncertain_duration=%s,uncertain_time_delay=%s,uncertain_along=%s,uncertain_cross=%s)" \
        % (self.current_cycle.fDuration, self.current_cycle.fUncertainStartTime, self.current_cycle.fUpCurUncertainty, self.current_cycle.fRightCurUncertainty)
        return info
    ...
    def set_ossm(self, CyOSSMTime ossm):
        """
        Takes a CyOSSMTime object as input and sets C++ CurrentCycle mover properties from the OSSM object.
        """
