...
# create_new_elements() should not be a Spill method
#  (maybe Spill_contianer or element_type class)
...
class Spill(object):
    ...
    .. note:: This class is not serializable since it will not be used
              in PyGnome. It does not release any elements
    """
    ...
    def __init__(self, num_elements=0, on=True, id=None):
        """
        Base spill class. Spills used by a gnome model derive from this
        base class
    ...
    def __copy__(self):
        ...
        Since it is a shallow copy of the dict, the _gnome_id object
        is not copied, but merely referenced.
...
class FloatingSpill(Spill):
    ...
    def __init__(self,
        ...
        windage_persist:
            The 0 or -1 means the persistence is infinite so it is only set
        ...
        :param windage_persist: Duration over which windage persists - this is
                                given in seconds. Default is 900s.
        :type windage_persist: integer

        .. note:: Remaining kwargs are passed onto Spill's __init__ using
                  super.  See base class documentation for remaining
                  valid kwargs.
        """
...
class PointSourceRelease(FloatingSpill, serializable.Serializable):
    ...
    """
    _update = ['start_position', 'release_time',
               'end_position', 'end_release_time']

    # not sure these should be user update able
    _create = ['num_released', 'not_called_yet',
               'prev_release_pos', 'delta_pos']
    _create.extend(_update)
    ...
    def __init__(self,
        ...
        :param end_position=None: optional -- for a moving source, the
                                  end position
        ...
        :param windage_range=(0.01, 0.04): the windage range of the elements
                                           default is (0.01, 0.04)
                                           from 1% to 4%.
        :type windage_range: tuple: (min, max)

        :param windage_persist=-1: Default is 900s, so windage is updated every
                                   900 sec.
                                   -1 means the persistence is infinite so it
                                   is only set at the beginning of the run.
        ...
        """
        super(PointSourceRelease, self).__init__(windage_range,
                                                 windage_persist,
                                                 **kwargs)
        ...
        if end_release_time is None:
            ...
            if release_time > end_release_time:
                raise ValueError("end_release_time must be greater" \
                                 " than release_time")
    ...
    """
    Following properties were added primarily for setting values correctly
    when json input form webgnome is convereted to dict which is then used by
    from_dict to update variables.
    In this case, if user does not set end_positions or end_release_time,
    they become None in the dict. If these are None, then they should be
    updated to match release_time and start_position.
    """
    @property
    ...
    def release_elements(self, current_time, time_step):
        ...
        if self.delta_release <= 0:
            ...
            if np.array_equal(self.delta_pos, (0.0, 0.0, 0.0)):
                ...
                arrays['positions'][:, 0] = np.linspace(self.start_position[0],
                                                        self.end_position[0],
                                                        num)
                arrays['positions'][:, 1] = np.linspace(self.start_position[1],
                                                        self.end_position[1],
                                                        num)
                arrays['positions'][:, 2] = np.linspace(self.start_position[2],
                                                        self.end_position[2],
                                                        num)
            return arrays
...
class SubsurfaceRelease(SubsurfaceSpill):
    ...
    def __init__(self,
        ...
        :param start_position: location the LEs are released
                               (long, lat, z) (floating point)
        :param release_time: time the LEs are released (datetime object)
        :param end_position=None: optional -- for a moving source,
                                  the end position
        :param end_release_time=None: optional -- for a release over time,
                                      the end release time
        """
...
class SpatialRelease(FloatingSpill):
    ...
    def __init__(self,
        ...
        :param windage=(0.01, 0.04): the windage range of the LEs  Default is
                                     from 1% to 4%.
        :param windage: tuple of floats: (min, max).

        :param persist=900: Default is 900s, so windage is updated every
                            900 sec. The -1 means the persistence is infinite
                            so it is only set at the beginning of the run.
        :type persist: integer secondsDefault
                       The -1 means the persistence is infinite so it is only
                       set at the beginning of the run.
        """
    ...
    def release_elements(self, current_time, time_step):
        ...
        .. note:: this releases all the elements at their initial positions
                  at the release_time
        """
