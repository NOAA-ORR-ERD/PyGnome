...
# ===============================================================================
    ...
    def draw_land(self):
        ...
        back_image = PIL.Image.new(self.image_mode, self.image_size,
            ...
            for p in polygons:
                if p.metadata[1].strip().lower() == 'map bounds':
                    if self.draw_map_bounds:
                        ...
                        poly = \
                            np.round(p).astype(np.int32).reshape((-1,
                                )).tolist()
                        drawer.polygon(poly,
                                outline=self.colors['map_bounds'])
                    ...
                    poly = np.round(p).astype(np.int32).reshape((-1,
                            )).tolist()
    ...
    def create_foreground_image(self):
        ...
        self.fore_image = PIL.Image.fromarray(self.fore_image_array,
                mode='P')
    ...
    def draw_elements(self, sc):
        ...
        if sc.num_released > 0:  # nothing to draw if no elements
            ...
            on_map = (pixel_pos[:, 0] > 1) & (pixel_pos[:, 1] > 1) \
            ...
            arr[(pixel_pos[not_on_land, 1] - 0.5).astype(np.int32),
                (pixel_pos[not_on_land, 0] - 0.5).astype(np.int32)] = \
                color
            arr[(pixel_pos[not_on_land, 1] - 0.5).astype(np.int32),
                (pixel_pos[not_on_land, 0] + 0.5).astype(np.int32)] = \
                color
...
class BW_MapCanvas(MapCanvas):
    ...
    colors_BW = [  # note:transparent not really supported
        ('transparent', 0),
        ('background', 0),
        ('lake', 0),
        ('land', 1),
