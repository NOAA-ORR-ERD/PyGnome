...
class SubstanceSchema(ObjTypeSchema):
    standard_density = SchemaNode(Float())


class WeatheringSubstance(GnomeId, OilProps):

    def serialize(self, options={}):
        return super(WeatheringSubstance, self).serialize(options=options)

    @classmethod
    def deserialize(cls, json_, refs=None):
        return super(WeatheringSubstance, cls).deserialize(json_, refs=refs)

    def initialize_LEs(self, num_released, data_arrays):
        return None
